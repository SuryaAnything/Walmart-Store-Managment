//
// Created by surya on 16.2.24.
//

#include "./include/wlmrt.h"
#include <string.h>
#include "./include/cJSON.h"
#include "./include/system.h"
#include "./include/map.h"
#include "./include/bPLUS_Tree.inl"

static int aisles_table[MAX_AISLES] = {0};
static int AISLES = 5;
WLMRT_DEPRECATED static Item_node *products[MAX_AISLES] = {0};
...
// WLMRT_DEPRECATED void wlmrt_refresh_product_list() {
...
bool wlmrt_check_availability(int id, int quantity, char* expiry) {
    ...
    if (nptr->quantity >= quantity) {
        if (expiry_ > item_expiry) { availability = false; }
    }
...
void wlmrt_merge_aisles(int aisles1, int aisles2) {
    ...
    if (aisles1 < aisles2){
        while (temp->ptr.local.next) { temp = temp->ptr.local.next; }
        ...
        while (temp2->ptr.local.next) { temp2 = temp2->ptr.local.next; }
...
Item_node * wlmrt_purchase_new_item(Cart* items, int no_of_items) {
    ...
    for (int iterator = 0; iterator < no_of_items; ++iterator) {
        ...
        if (node) {
            int no = item.count;
            node->quantity -= no;
            if (node->quantity < 0) { printf("Sufficient no not available\n"); node->quantity += no; }
            else {
    ...
    while (temp) {
        ...
        while (list && !check) {
            for (int i = 0; i < list->count; ++i) {
                if (list->data.val[i]->item_id == item_id) { check = 1; break;  }
            }
...
void add_to_bill(const char* name, bPLUS *cart) {
    FILE *fp = fopen("../database/bill.json", "r");
    cJSON *root = NULL;
    ...
    FILE *fptr = fopen("../database/bill.json", "w");
    if (fptr == NULL) {
...
void wlmrt_write_products_list() {
    FILE *fptr = fopen("../database/items.json","w");
    if (fptr == NULL) {
        printf("Error opening File\n");
        return;
    }
    ...
    if (item_id_1 / ID_FORMAT == item_id_2 / ID_FORMAT) {
        ...
        while (first && found!=0) {
            for (int i = 0; i < first->count && found; ++i) {
                if (first->data.val[i]->item_id == min_id) { start = 1; }
                if (start) {
                    wlmrt_display_item(first->data.val[i]);
                }
                if (first->data.val[i]->item_id == max_id) { found = 0; }
            }
            first = first->ptr.local.next;
        }
    }
